...
\end{enumerate}
For the mechanisms NAHR and NHR, SV breakpoints will be placed next to the repeat regions (no overlap). For VNTR, TEI and Other, the breakpoints lie within the repeats. \\ This feature is turned off automatically, when the user specifies his own genome (i.e. any genome other than hg19). \\
...
For example
<<label=weightsExample, eval=FALSE, echo=TRUE, hide=FALSE>>=
...
\end{itemize}
In both cases, RSVSim saves the coordinates as \Rcode{RData} object \Rcode{repeats\_hg19.RData} to the RSVSim installation directory for a faster access in the future (if write privileges allow to do so). Then, one of the two steps mentioned above is not necessary anymore and \Rpackage{RSVSim} loads the coordinates automatically from the \Rcode{RData} file.
